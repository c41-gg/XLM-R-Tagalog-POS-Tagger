...
Usage:
    ...
    # effective batch size = 4 * 4 = 16, while only ever holding 4 sequences
    # in VRAM at once -- tuned for a 4GB-class GPU (e.g. RTX 3050 laptop).
    # If you still hit CUDA OOM: lower --batch_size to 2 and raise
    # --gradient_accumulation_steps to 8 (same effective batch size, less VRAM).
...
def main():
    ...
    parser.add_argument("--save_total_limit", type=int, default=1,
                         help="How many recent checkpoints to keep on disk. Each "
                              "xlm-roberta-base checkpoint (weights + AdamW optimizer "
                              "state) is several GB -- default is 1 to minimize disk "
                              "usage; raise it if you want more rollback points and "
                              "have the space.")
    parser.add_argument("--resume_from_checkpoint", nargs="?", const=True, default=None,
                         help="Resume an interrupted run. Pass with no value to "
                              "auto-find the latest checkpoint-* in --output_dir, "
                              "or pass an explicit path to a specific checkpoint dir.")
    ...
    if args.resume_from_checkpoint:
        print(f"[resume] resuming from: "
              f"{'auto-detected latest checkpoint in output_dir' if args.resume_from_checkpoint is True else args.resume_from_checkpoint}")
